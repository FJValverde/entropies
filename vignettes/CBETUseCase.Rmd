...
```{r}
X <- as.matrix(ds) #as.matrix(dataset[,-dsRow$className])
Y <- class.ds #dataset[, dsRow$classVar]
...
gp <- ggmetern(data=experiments, fancy) +
    geom_point(aes(colour=Phase), size=1)  +
    scale_colour_brewer(palette="Set1")
gp
```

Note that there is a suspicious behaviour in the plot in that the classifier achieves a better information transfer (correlated with accuracy) in test than in training. 
...
for (i in 1:numFolds){
    for (m in models){
        ...
        results <- rbind(results, 
                         cbind(jentropies(trainCM$table), Fold=i,method=m, Phase="train"),
                         cbind(jentropies(testCM$table), Fold=i, method=m, Phase="test")
        )
...
eT <- ggmetern(data=results, fancy) + 
    geom_point(aes(colour=Phase, shape=method), size=3)  +
    scale_colour_manual(values=c("black","red")) # Don't trust the training, that is the red
eT
