...
m <- 
    rbind(
    c("irredundant", 0.05, 0.05, 0.9, "irredundant"), 
    c("redundant", 0.05, 0.9, 0.05, "redundant"),
    c("constant", 0.9, 0.05, 0.05, "constant")
)
d <- data.frame(
    name = m[,1],
...
names(d) <-  c(multiSplitEntropies, "point")
```

```{r}
conceptual <- ggmetern(d) + 
...
et <- ggmetern(eds) #+ # now create the plot
...
et <-  et +
    ...
    #scale_fill_discrete() 
# from http://stackoverflow.com/questions/9690648/point-clipped-on-x-axis-in-ggplot
#et$layout$clip[et$layout$name=="panel"] <- "off" #does not work!
et
