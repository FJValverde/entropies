...
m <- sapply(name, function(n){nrow(evalDataset(n))}) # Number of instances
n <- sapply(name, function(n){ncol(evalDataset(n))}) - 1 # Number of features
datasets <- data.frame(name, className, K=as.integer(K), n=as.integer(n), m, stringsAsFactors=FALSE)
datasets
