...
# dsName <- "Sonar"
# Choose: "iris", "BreastCancer" or "Glass"
...
if (fancy){
    ...
    #orderingColors <- rev(cm.colors(ncols+1))[1:ncols + 1]#too washed
...
#X for the x and circle for Y
typeShapes <- c("X" = 4, "Y" = 6, "ALL" = 10, "XY" = 20)
#X for the x and nabla for Y
#typeShapes <- c("X" = "x", "Y" = "y", "ALL" = "A", "XY" = "o")
```
...
if (dsName != "Ionosphere"){
    ...
    sedf <- rbind(sedf,cbind(data=paste0("1_",ncols), transform="log", 
...
transfo_smet <- 
    ...
    scale_fill_manual(values=orderingColors) + 
    #scale_fill_brewer(palette=orderingPalette, guide=guide_legend()) +
    #scale_colour_brewer(palette=orderingPalette) +
    #labs(fill="Feature set", shape="Source type")
    guides(fill=guide_legend(override.aes=list(shape=22))) + #show a shape than can be filled!
    theme(legend.key=element_blank())
transfo_smet
if (getPlot){
