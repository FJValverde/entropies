...
MLC with mlr (now obsolete, see mlr3) and the SMET and CMET.

Original from "5/15/2017" at least.
...
Rolling back ggplot2 to a version where ggtern works:

```{r}
rollbackGgplot2 <- TRUE
# rollbackGgplot2 <- FALSE
# # FVA 9/11/2022: 
# # ggplot2_3.3.6.tgz seems to be the last version that was compatible with ggtern
if (rollbackGgplot2){
# download.file("http://cran.r-project.org/src/contrib/Archive/ggplot2/ggplot2_3.3.6.tar.gz",
# dest="ggplot2_3.3.6.tar.gz")
# install.packages("ggplot2_3.3.6.tar.gz",repos=NULL,type="source")

#ON 15/10/25 is seems to be ggplot2_3.5.0
download.file("http://cran.r-project.org/src/contrib/Archive/ggplot2/ggplot2_3.5.0.tar.gz",
dest="ggplot2_3.5.0.tar.gz")
...
library(mlr)#Obsolete. Substitute by ml3
...
```{r import code}
#library(utiml)#no longer available!See above for direct importing. 
#source('my_utiml.R', echo = F)#FVA: this is too lengthy!
```
...
```{r data}
yeast <- mlr::getTaskData(yeast.task)
...
# lrn.rfsrc
lrn.rFerns = mlr::makeLearner("multilabel.rFerns")
lrn.rFerns
```
